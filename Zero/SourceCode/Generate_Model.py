...
def GenerateP300Data(filename):
        target = np.zeros((300,32,76))
        nontarget = np.zeros((1500,32,76))
        for i in np.arange(1,3):
            ...
            eegData = Re_referencing(eegData[ChannelInx], channelNum, sampleNum)
...
def main():
        ...
        Datapath = 'C:/Users/wldk5/WorldSystem/Zero/CNNdata'
        createFolder(Datapath)
        createFolder(Datapath + '/PythonData')
        
        saveTargetroot = Datapath + '/PythonData/alltarget.out'
        saveNontargetroot = Datapath + '/PythonData/allnontarget.out'
        root = 'C:/Users/wldk5/WorldSystem/Zero/CNNdata/55P300data/MatlabData/S'
        filename = ''
        alltarget = np.zeros((300*55,32,76))
        allnontarget = np.zeros((1500*55,32,76))
...
#        trainingData = np.concatenate((target,nontarget),axis = 0);
        ...
        trainingData = np.reshape(trainingData,(1800*55,1,32,76));
        
        label = np.concatenate((np.ones(300*55),np.zeros(1500*55)),0);
        label = np_utils.to_categorical(label, 2);
        
        ##Generating CNN model
        model = Sequential();
        #model.add(AveragePooling2D(pool_size=(1, 4), strides=(1,4))) # this was added
        model.add(Conv2D(76, kernel_size=(1, 25),data_format='channels_first',input_shape=(1, 32, 76)))
        model.add(BatchNormalization())
        model.add(Conv2D(76, (32, 1),data_format='channels_first'))
        model.add(BatchNormalization())
        model.add(Flatten())
        model.add(Dense(2))
        ...
        data = trainingData;
        randIdx = np.random.permutation(55*1800);
        trainIdx = randIdx[0:int(1800*55*0.95)];
        valIdx = randIdx[int(1800*55*0.95):55*1800];
...
#        model.save_weights('C:/Users/wldk5/WorldSystem/Zero/ZeroModel/ZeroCNNWeight4.h5')
