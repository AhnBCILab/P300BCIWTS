...
def DownsamplingOnlineEpoch(Epochs1, Epochs2, Epochs3, Epochs4, Epochs5, Epochs6, Epochs7, downsampleRate):
        ...
        Downsampled7 = np.zeros((Epochs7.shape[0],Epochs7.shape[1],num))
        for i in range(num):
            for j in range(Epochs1.shape[1]):
                for k in range(Epochs1.shape[0]):
                    ...
                    Downsampled7[k,j,i] = np.mean(Epochs7[k,j,i*downsampleRate:(i+1)*downsampleRate],dtype=np.float64)
        return [Downsampled1, Downsampled2, Downsampled3, Downsampled4, Downsampled5, Downsampled6, Downsampled7, num]

def Online_Convert_to_featureVector(Epochs1, Epochs2, Epochs3, Epochs4, Epochs5, Epochs6, Epochs7, Num, featureNum):
        ...
        Features7 = np.zeros((Num, featureNum))
        for i in range(Num):
            Features7[i,:] = np.reshape(Epochs7[i,:,:],(1,featureNum))
...
#        global file_exist, file1, file2, channelNum
        ...
        for i in range(0, 12):
            ...
            #Downsampling Epochs
            [Epochs1,Epochs2,Epochs3,Epochs4,Epochs5,Epochs6,Epochs7,epochSampleNum] = DownsamplingOnlineEpoch(Epochs1, Epochs2, Epochs3, Epochs4, Epochs5, Epochs6, Epochs7, downsampleRate)
            samplingFreq = samplingFreq/4
            
            #Convert a feature vector
            featureNum = channelNum*epochSampleNum
            [Features1, Features2, Features3, Features4, Features5, Features6, Features7] = Online_Convert_to_featureVector(Epochs1, Epochs2, Epochs3, Epochs4, Epochs5, Epochs6, Epochs7, Num1, featureNum)        
            ...
            Features7 = Features7[:,SelectedFeatures]
            ...
            but7_pred = lda.predict(Features7)
